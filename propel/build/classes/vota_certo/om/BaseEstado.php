...
abstract class BaseEstado extends BaseObject implements Persistent
{
    ...
    /**
     * @var        PropelObjectCollection|Cidade[] Collection to store aggregation of Cidade objects.
     */
    protected $collCidades;
    protected $collCidadesPartial;
    ...
    public function setNome($v)
    {
        ...
        if ($this->nome !== $v) {
            $this->nome = $v;
            $this->modifiedColumns[] = EstadoPeer::NOME;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false)
    {
        try {
            ...
            return $startcol + 2; // 2 = EstadoPeer::NUM_HYDRATE_COLUMNS.
    ...
    public function delete(PropelPDO $con = null)
    {
        ...
        try {
            $deleteQuery = EstadoQuery::create()
    ...
    protected function doInsert(PropelPDO $con)
    {
        ...
        if ($this->isColumnModified(EstadoPeer::ID)) {
            $modifiedColumns[':p' . $index++]  = '`id`';
        }
        if ($this->isColumnModified(EstadoPeer::NOME)) {
    ...
    {
        if (isset($alreadyDumpedObjects['Estado'][$this->getPrimaryKey()])) {
            return '*RECURSION*';
        }
        $alreadyDumpedObjects['Estado'][$this->getPrimaryKey()] = true;
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(EstadoPeer::NOME)) $criteria->add(EstadoPeer::NOME, $this->nome);
    ...
    public function clearCidades()
    {
        ...
        $this->collCidadesPartial = null;

        return $this;
    }

    /**
     * reset is the collCidades collection loaded partially
     *
     * @return void
     */
    public function resetPartialCidades($v = true)
    {
        $this->collCidadesPartial = $v;
    }
     ...
     * If this Estado is new, it will return
     * an empty collection or the current collection; the criteria is ignored on a new object.
     *
     * @param Criteria $criteria optional Criteria object to narrow the query
     * @param PropelPDO $con optional connection object
     * @return PropelObjectCollection|Cidade[] List of Cidade objects
     * @throws PropelException
     */
    public function getCidades($criteria = null, PropelPDO $con = null)
    {
        $partial = $this->collCidadesPartial && !$this->isNew();
        if (null === $this->collCidades || null !== $criteria  || $partial) {
            if ($this->isNew() && null === $this->collCidades) {
                ...
                $collCidades = CidadeQuery::create(null, $criteria)
                    ...
                    ->find($con);
                if (null !== $criteria) {
                    if (false !== $this->collCidadesPartial && count($collCidades)) {
                      $this->initCidades(false);

                      foreach ($collCidades as $obj) {
                        if (false == $this->collCidades->contains($obj)) {
                          $this->collCidades->append($obj);
                        }
                      }

                      $this->collCidadesPartial = true;
                    }

                    $collCidades->getInternalIterator()->rewind();

                    return $collCidades;
                }

                if ($partial && $this->collCidades) {
                    foreach ($this->collCidades as $obj) {
                        if ($obj->isNew()) {
                            $collCidades[] = $obj;
                        }
                    }
    ...
    {
        $cidadesToDelete = $this->getCidades(new Criteria(), $con)->diff($cidades);


        $this->cidadesScheduledForDeletion = $cidadesToDelete;

        foreach ($cidadesToDelete as $cidadeRemoved) {
            $cidadeRemoved->setEstado(null);
        }
    ...
    public function addCidade(Cidade $l)
    {
        ...
        if (!in_array($l, $this->collCidades->getArrayCopy(), true)) { // only add it if the **same** object is not already associated
            $this->doAddCidade($l);

            if ($this->cidadesScheduledForDeletion and $this->cidadesScheduledForDeletion->contains($l)) {
                $this->cidadesScheduledForDeletion->remove($this->cidadesScheduledForDeletion->search($l));
            }
        }
    ...
    /**
     * @param	Cidade $cidade The cidade object to remove.
     * @return Estado The current object (for fluent API support)
     */
    public function removeCidade($cidade)
    {
        if ($this->getCidades()->contains($cidade)) {
            $this->collCidades->remove($this->collCidades->search($cidade));
            if (null === $this->cidadesScheduledForDeletion) {
                $this->cidadesScheduledForDeletion = clone $this->collCidades;
                $this->cidadesScheduledForDeletion->clear();
            }
            $this->cidadesScheduledForDeletion[]= clone $cidade;
    ...
    {
        return (string) $this->exportTo(EstadoPeer::DEFAULT_STRING_FORMAT);
    }
